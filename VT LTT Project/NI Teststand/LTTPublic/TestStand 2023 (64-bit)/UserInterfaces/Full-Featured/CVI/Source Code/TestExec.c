...
HRESULT CVICALLBACK ApplicationMgr_OnReportError(CAObjHandle caServerObjHandle, void *caCallbackData, long  errorCode, char *errorMessage)
{
	...
	{
		strncpy(errMsg, errorMessage, ERRMSG_SIZE);   // update global errMsg buffer
		errMsg[ERRMSG_SIZE - 1] = '\0';
		DisplayError(errorCode);
	}
	else
	{
		char title[1024] = ""; // 1024 is just more than enough rooom for any reasonable title.
		
		// No tsErrChk here.  If we fail to get the resource string we still want to display our error dialog with the default title.
		TS_GetResourceString(gMainWindow.engine, "TSUI_OI_MAIN_PANEL", "ERR_BOX_TITLE", "ERROR", sizeof(title), title);

		tsErrChk(TS_EngineDisplayErrorDialog(gMainWindow.engine, &errorInfo, title, errorMessage, errorCode, 0));
	}
...
{
	char		oiVersionString[MAX_VERSION_STRING_SIZE];
	int			error = 0;
	char		*string = NULL;
	...
	strncpy( oiVersionString, "<YEAR> (<MAJOR>.<MINOR>.<PATCH>.<BUILD>)", MAX_VERSION_STRING_SIZE); // <--- YOUR VERSION HERE.  (This is the version displayed in the about box.)   
	...
	errChk( AppendString(&string, oiVersionString, -1));
